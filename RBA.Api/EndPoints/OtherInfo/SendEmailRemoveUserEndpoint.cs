...
[HttpPost("emailtoremove"), AllowAnonymous]
public class SendEmailRemoveUserEndpoint(
...
) : Endpoint<EntityRequestById>
{
  ...
  public override async Task HandleAsync(EntityRequestById req, CancellationToken ct)
  {
    ...
    entity.Email_Notification = "sent for revoke";
  ...
  private string PopulateBody(V_UserRoleAllInfo allInfo, IEnumerable<OtherInfo> apprvInfo)
  {
    ...
    using (StreamReader reader = new($"{emailPath}/email_to_remove_user.html"))
    {
    ...
    var userName = $"{allInfo.User_First_Name} {allInfo.User_Last_Name}";
    body = body.Replace("{user_name}", userName);

    body = body.Replace("{user_cd}", allInfo.User_Cd);
